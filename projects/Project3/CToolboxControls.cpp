...
void CToolboxControls::CreateControl(UIHandleWin hwin, UIHandle hcs,TYP typ, LPCWSTR nm )
{
	...
	obj.wboard.drawFunc = [](LPVOID captureobj, D2DContext& cxt) {
		...
		{
			obj.rc = obj.acordion[TESTTEST-1];
			obj.show_stat -= 2;
			obj.acordion_idx = 0;
		}
		...
		if ( obj.show_stat == 2 || obj.show_stat == 3 )
		{
			...
			cxt.DDrawRect(rc.ZeroRect(), D2RGB(220,220,220),D2RGB(220,220,220) );
		...
		{
			D2DSetRect(obj.me, obj.rc ); // mat‚Ì		
			cxt.DDrawRect(obj.rc.ZeroRect(), D2RGB(220,220,220),D2RGB(220,220,220) );
		}
	...
	obj.rc = rc;
	auto whb2 = D2DCreateClientControls(&obj, obj.wboard.drawFunc, obj.wboard.procFunc, hwin, hcs, rc, STAT_VISIBLE | STAT_ENABLE, nm, 0);
